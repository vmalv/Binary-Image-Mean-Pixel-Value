...
input_file_path = str(Path(__file__).parent / "input" / (file + ext))
raw = Image.open(input_file_path)
...
# defined above.
output_file_path = str(Path(__file__).parent / "output" / f"Binary_{file}.png")
binary.save(output_file_path, "PNG", dpi=(dpi, dpi))
print(f"Binarized image exported to the file: {output_file_path}")
